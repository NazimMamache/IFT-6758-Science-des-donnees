...
def filter_df(csv_path: str, label: str) -> List[str]:
    """
    Écrivez une fonction qui prend le path vers le csv traité à partir de q1 et renvoie un df avec seulement les rangées qui contiennent l'étiquette `label`.
    ...
    for i in range(len(df)):
        if label not in df['label_names'][i].split("|"):
            list.append(i)
    return df.drop(list)
...
def data_pipeline(csv_path: str, label: str) -> None:
    ...
    """
    os.makedirs(label + '_raw', exist_ok = True)
    os.makedirs(label + '_cut', exist_ok = True)
    df = filter_df(csv_path, label)
    for i in tqdm(df.index):
        download_audio(df['# YTID'][i], label + '_raw/'+ df['# YTID'][i] + '.mp3')
        if os.path.exists(label + '_raw/'+ df['# YTID'][i] + '.mp3'):
            cut_audio(label + '_raw/' + df['# YTID'][i] + '.mp3',   label + '_cut/'+ df['# YTID'][i] + '.mp3', df[' start_seconds'][i], df[' end_seconds'][i])
    return None
...
def rename_files(path_cut: str, csv_path: str) -> None:
    ...
    """
    df = pd.read_csv(csv_path)
    for count, filename in enumerate(os.listdir(path_cut)):
        df_copy = df[ df['# YTID'] == filename[:-4] ].reset_index()
        start = str(int(df_copy[' start_seconds'][0]))
        end = str(int(df_copy[' end_seconds'][0]))
        duration = str(int(float(end) - float(start)))
        string = re.sub("(.).mp3",r'\1'+ '_' + start + '_' + end + '_' + duration + ".mp3", filename)
        os.rename(path_cut + '/' + filename, path_cut + '/' + string)
    return None
