...
def bin_counts(df: pd.DataFrame, bins: list, bin_centers: list) -> pd.DataFrame:
    """ Renvoie une trame de données avec les `ratings` regroupés entre les valeurs données dans `bins`, et
...
# TODO - complétez cette méthode
def sample_two_groups(df: pd.DataFrame, n_overrep: int, n_underrep: int, n_iter: int=1000) -> Tuple[np.array, np.array]:
